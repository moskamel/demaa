    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 380, background: 'var(--surface-1)', borderRight: '1px solid var(--hairline)', zIndex: 201, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--hairline)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)' }}>طلب #{order.id}</div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 4, padding: '3px 10px', borderRadius: 20, background: `${statusColor}18`, border: `1px solid ${statusColor}40` }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: statusColor }}>{statusLabels[order.status]}</span>
            </div>
          </div>
        ...
        <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {order.riskScore && order.riskScore >= 60 && (
            <div style={{ background: order.riskScore >= 80 ? 'rgba(255,85,119,0.08)' : 'rgba(255,122,61,0.08)', border: `1px solid ${order.riskScore >= 80 ? 'rgba(255,85,119,0.3)' : 'rgba(255,122,61,0.3)'}`, borderRadius: 12, padding: '12px 14px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--ink-muted)', lineHeight: 1.5 }}>{order.suspiciousReason}</div>
              </div>
            </div>
          )}
          ...
          <Section title="معلومات العميل">
            <Row icon={<span style={{ fontSize: 15 }}>👤</span>} label="الاسم" value={order.customer} />
            <Row icon={<Phone size={13} color="var(--ink-muted)" />} label="الجوال" value={order.phone || '—'} ltr />
          ...
          <Section title="المنتجات">
            {order.items.map((item, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < order.items.length - 1 ? '1px solid var(--hairline-soft)' : 'none' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{(item.price * item.qty).toLocaleString('ar-SA')} ر.س</div>
              </div>
            ))}
          ...
          <Section title="الدفع والشحن">
            <Row icon={<CreditCard size={13} color="var(--ink-muted)" />} label="طريقة الدفع" value={paymentLabels[order.payment] || order.payment} />
            <Row icon={<Clock size={13} color="var(--ink-muted)" />} label="تاريخ الطلب" value={new Date(order.createdAt).toLocaleDateString('ar-SA', { year: 'numeric', month: 'long', day: 'numeric' })} />
          ...
          {order.status === 'pending' && (
            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
              <button style={{ flex: 1, padding: '10px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, background: '#22c55e', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <CheckCircle size={14} /> قبول
              </button>
              <button style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1px solid rgba(255,85,119,0.3)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, background: 'rgba(255,85,119,0.08)', color: '#ff5577', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <XCircle size={14} /> رفض
              </button>
            </div>
          )}
