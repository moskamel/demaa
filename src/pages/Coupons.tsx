...
function formatVal(c: CouponData) {
  return c.type === 'percentage' ? `${c.value / 100}%` : `${(c.value / 100).toLocaleString('en-US')} $`
}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div>
                    <label style={{ fontSize: 12, color: 'var(--ink-muted)', marginBottom: 6, display: 'block' }}>
                      {form.type === 'percentage' ? 'نسبة الخصم (%)' : 'مبلغ الخصم ($)'}
                    </label>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--ink-muted)' }}>
                      ...
                      {c.expiresAt ? ` · ينتهي ${new Date(c.expiresAt).toLocaleDateString('en-US')}` : ''}
                    </div>
                  </div>
