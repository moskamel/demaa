...
export function generateResponse(
  ...
  if (intent === 'get_pending_orders') {
    ...
    return {
      role: 'deema',
      content: `عندك ${orders.length} طلب معلق${entities.city ? ` في ${entities.city}` : ''} — إجمالي ${fmt(total)} $${cashCount > 0 ? `\n\n⚠️ ${cashCount} طلبات بالكاش — تأكد قبل القبول` : ''}${riskCount > 0 ? `\n🔴 ${riskCount} طلبات مشبوهة تحتاج مراجعة` : ''}${insight}`,
  ...
  if (intent === 'get_accepted_orders') {
    ...
    return {
      role: 'deema',
      content: `عندك ${orders.length} طلب مقبول جاهز للشحن — ${fmt(total)} $`,
  ...
  if (intent === 'filter_orders_city' && entities.city) {
    ...
    return {
      role: 'deema',
      content: `طلبات ${entities.city}: ${all.length} طلب — ${fmt(total)} $${topCityNote}`,
  ...
  if (intent === 'add_product') {
    ...
    if (!price) return { role: 'deema', content: `ما عرفت السعر. مثال: "ضيف منتج عطر اسمه العود الفاخر سعره 280 $"` }
  ...
  if (intent === 'update_price') {
    ...
    const action = percent && percent < 0 ? `تخفيض ${Math.abs(percent)}%` : percent ? `رفع ${percent}%` : `تغيير السعر لـ ${entities.price} $`

    return {
      role: 'deema',
      content: `${action} على ${targetProducts.length} منتج${category ? ` في فئة "${category}"` : ''}:\n\n${targetProducts.map(p => `• ${p.name}: ${fmt(p.price)} → ${fmt(calcNewPrice(p.price, percent, entities.price))} $`).join('\n')}\n\nتأكيد التغيير؟`,
  ...
  if (intent === 'get_analytics') {
    ...
    const topCityLine = topCityData ? `\n\n💡 ${topCity}: ${topCityData.orders} طلب — ${fmt(topCityData.revenue)} $ (المدينة الأعلى ربحية)` : ''
    ...
    return {
      ...
      stats: [
        ...
        { n: fmt(data.revenue), l: '$ إيراد', c: 'var(--ink)' },
